...
import { signInWithGitHub, signOut, isAdmin, getCurrentUser } from '../lib/auth';
      ...
      <div className="page-shell">
        <main className="content">
          <section className="panel" style={{ textAlign: 'center', padding: '48px 24px' }}>
            <p className="section-label">管理者用</p>
            <h2>作品を編集するには GitHub でログインしてください。</h2>
            <button
              type="button"
              style={{ marginTop: '24px', fontSize: '1rem', padding: '14px 32px' }}
              onClick={signInWithGitHub}
            >
              GitHub でログイン
            </button>
            <p style={{ marginTop: '16px' }}>
              <a href="/" style={{ color: '#22584c' }}>← 作品一覧を見る</a>
            </p>
          </section>
        </main>
      </div>
  ...
  if (!isAdminUser) {
    return (
      <div className="page-shell">
        <main className="content">
          <section className="panel" style={{ textAlign: 'center', padding: '48px 24px' }}>
            <p className="section-label">アクセス拒否</p>
            <h2>このアカウントでは編集権限がありません。</h2>
            <p style={{ marginTop: '16px' }}>
              <button type="button" className="text-button" onClick={signOut} style={{ marginTop: '16px' }}>ログアウト</button>
            </p>
            <p style={{ marginTop: '16px' }}>
              <a href="/" style={{ color: '#22584c' }}>← 作品一覧を見る</a>
            </p>
  ...
  return (
    <div className="page-shell">
      <aside className="sidebar">
        <div className="eyebrow">Munenori IIDA / Portfolio Admin</div>
        <h1>作品登録</h1>
        <p className="lead">
        ...
        <div className="stat-list">
          <div className="stat-card">
            <span>作品</span>
            <strong>{sortedWorks.length} 件</strong>
          </div>
          <div className="stat-card">
            <span>保存先</span>
            <strong>{hasSupabase ? 'Supabase 優先' : 'LocalStorage 代替'}</strong>
          </div>
          <div className="stat-card">
            <span>公開</span>
        ...
        <div style={{ marginTop: '14px' }}>
          <p>ログイン中: <strong>{user?.user_metadata?.user_name}</strong></p>
          <button type="button" className="text-button" onClick={signOut}>ログアウト</button>
          <br />
          <a href="/" style={{ color: '#22584c', fontSize: '0.88rem', marginTop: '8px', display: 'inline-block' }}>← 作品一覧を見る</a>
        </div>
      </aside>
